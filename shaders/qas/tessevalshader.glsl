...
vec3 getBezierPos(float u, float v, float w, vec3 p0, vec3 p1, vec3 p2, vec3 e0, vec3 e1, vec3 e2)
{
    ...
    return w * (p0 * w + edgePoint(e0, p0, p1) * 2 * v) +
           u * (p1 * u + edgePoint(e1, p1, p2) * 2 * w) +
           v * (p2 * v + edgePoint(e2, p0, p2) * 2 * u);
}
...
{ 
    ...
    tePosition = interpolate(controlNet[0], controlNet[1], controlNet[2]);
    
    // Also, we'll need to calculate the correct normals
    //new_normal = normalize(normalmatrix * interpolate(controlNetNormals[0], controlNetNormals[1], controlNetNormals[2]));
